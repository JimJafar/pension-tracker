...
import { PensionWithTotals, Holding, HoldingWithPrice } from '../types/pension';
...
const DashboardPage: React.FC = () => {
  ...
  const [pensionHoldings, setPensionHoldings] = useState<{ [pensionId: number]: Holding[] }>({});
  ...
  const enrichedPensions = pensions.map((pension) => {
    ...
    const holdingsWithPrices: HoldingWithPrice[] = holdings.map((holding) => {
      ...
      const totalValue = currentPrice ? currentPrice * holding.shares : undefined;
