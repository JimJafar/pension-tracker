...
router.get('/pensions/:pensionId/holdings', async (req: Request, res: Response): Promise<void> => {
  try {
...
// Create holding
router.post('/pensions/:pensionId/holdings', async (req: Request, res: Response): Promise<void> => {
  try {
    const pensionId = parseInt(req.params.pensionId);
    ...
    const existing = await HoldingModel.findByPensionIdAndTicker(pensionId, ticker);
    if (existing) {
      res.status(400).json({ error: 'Holding for this ticker already exists' });
      return;
    }
...
// Update holding
router.put('/holdings/:id', async (req: Request, res: Response): Promise<void> => {
  try {
...
// Delete holding
router.delete('/holdings/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseInt(req.params.id);
    const holding = await HoldingModel.findById(id);
