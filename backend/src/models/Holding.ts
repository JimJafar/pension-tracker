...
export const HoldingModel = {
  ...
  findByPensionIdAndTicker: async (pensionId: number, ticker: string): Promise<Holding | undefined> => {
  ...
  create: async (data: {
    ...
    const { pension_id, ticker, shares } = data;
    return insert(
      'INSERT INTO holdings (pension_id, ticker, shares) VALUES (?, ?, ?)',
      [pension_id, ticker.toUpperCase(), shares]
    );
  },
